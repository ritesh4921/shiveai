...
export const ContactPage: React.FC<{ onBack: () => void }> = ({ onBack }) => (
  <LegalLayout title="Contact Us" icon={Mail} lastUpdated={new Date().toLocaleDateString()} onBack={onBack}>
    ...
    <div className="bg-indigo-50 p-6 rounded-xl border border-indigo-100 not-prose mb-8">
      <h3 className="text-indigo-900 font-bold text-lg mb-2">General Inquiries & Feedback</h3>
      <p className="text-indigo-700 mb-4">For general support, suggestions, or feedback about our tools.</p>
      <a href="mailto:mauryaritesh40@gmail.com" className="inline-flex items-center gap-2 text-white bg-indigo-600 hover:bg-indigo-700 px-4 py-2 rounded-lg font-medium transition-colors">
        <Mail className="w-4 h-4" /> Send Feedback Email
      </a>
    </div>
